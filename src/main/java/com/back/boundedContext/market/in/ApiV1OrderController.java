...
import com.back.shared.out.TossPaymentsService;
...
public class ApiV1OrderController {
    ...
    public record ConfirmPaymentByTossPaymentReqBody(
    ...
    public RsData<Void> confirmPaymentByTossPayments(
            @PathVariable int id,
            @Valid @RequestBody ConfirmPaymentByTossPaymentReqBody reqBody
    ){
        ...
        if (order.isPaymentInProgress())
            throw new DomainException("400-2", "이미 결제 진행 중인 주문입니다.");
